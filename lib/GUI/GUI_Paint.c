...
void Paint_SetPixel(UWORD xPoint, UWORD yPoint, UWORD color) {
    ...
    // Translate the pixel location to match image rotation
    switch(Paint.Rotate) {
        case 0:
            x = xPoint;
            y = yPoint;
    ...
    // Translate the pixel location to match mirrored image
    switch(Paint.Mirror) {
        case MIRROR_NONE:
            break;
    ...
    // Check pixel is within bounds
    if(x > Paint.WidthMemory || y > Paint.HeightMemory){
        Debug("Exceeding display boundaries\r\n");
        return;
    }

    // Apply the pixel to the image
    if(Paint.Scale == 2){
        ...
        Paint.Image[Addr] = Rdata | ((color << 4) >> ((y % 2)*4));
